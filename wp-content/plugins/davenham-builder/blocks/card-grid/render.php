		...
		<div class="card_grid">
			<?php foreach ( $cards as $card ) : ?>
				<article class="card_grid__item">
					<?php $card_alt = ! empty( $card['title'] ) ? $card['title'] : ''; ?>
					<?php if ( ! empty( $card['imageId'] ) ) : ?>
						<div class="card_grid__image"><?php echo wp_get_attachment_image( (int) $card['imageId'], 'large', false, array( 'alt' => esc_attr( $card_alt ) ) ); ?></div>
					<?php elseif ( ! empty( $card['imageUrl'] ) ) : ?>
						<div class="card_grid__image"><img src="<?php echo esc_url( $card['imageUrl'] ); ?>" alt="<?php echo esc_attr( $card_alt ); ?>" /></div>
					<?php endif; ?>
