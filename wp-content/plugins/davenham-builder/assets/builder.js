...
( function () {
	...
	BLOCKS.push(
		...
		{
			type: 'davenham/shop-hero',
			label: 'Shop Hero',
			icon: '🏞',
			category: 'hero',
			desc: 'Large retail-style shop hero with optional background image, gradient overlay, and CTAs.',
			defaults: {
				eyebrow: 'Davenham Scouts shop',
				heading: 'Every penny stays with us.',
				subtext: 'Tickets, kit and fundraising for 1st Davenham Scouts. Skip the postage — pick up free at Peckmill.',
				imageUrl: '',
			...
			fields: [
				...
				{ key: 'subtext',        label: 'Subtext',         type: 'textarea' },
				{ key: 'imageUrl',       label: 'Background image (URL)', type: 'image', help: 'Optional. If blank, a brand gradient is used instead.' },
				{ key: 'primaryText',    label: 'Primary CTA label', type: 'text' },
				{ key: 'primaryUrl',     label: 'Primary CTA URL',   type: 'text' },
				{ key: 'secondaryText',  label: 'Secondary CTA label', type: 'text' },
				{ key: 'secondaryUrl',   label: 'Secondary CTA URL',   type: 'text' },
			],
		},
		{
			type: 'davenham/section-shop-grid',
			label: 'Shop by Section',
			icon: '🦫',
			category: 'callout',
			desc: 'Six tiles — Squirrels, Beavers, Cubs, Scouts, Explorers, Network — in each section\'s brand colour with its logo.',
			defaults: {
				heading: 'Shop by section',
				subtitle: 'Pick a section to find kit, tickets and resources for that age group.',
			},
			fields: [
				{ key: 'heading',  label: 'Heading',  type: 'text' },
