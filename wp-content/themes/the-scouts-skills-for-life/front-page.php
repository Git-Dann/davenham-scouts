...
while ( have_posts() ) :
	the_post();
	$raw          = get_the_content();
	$has_builder  = ( false !== strpos( $raw, '<!-- wp:davenham/' ) );
?>
...
<?php if ( $has_builder ) : ?>
	...
	<?php // Builder-driven homepage — each block manages its own full-width section. ?>
	<div class="scouts-home cf">
		<?php echo apply_filters( 'the_content', $raw ); ?>
	</div>

<?php else : ?>

	<?php
	// ── Legacy fallback: the original hand-parsed homepage, kept so nothing
	// breaks if the page content is ever reset to plain blocks. ──
	...
	<div class="white_container cf">
		...
		<section class="welcome_section">
			<div class="wrapper cf">
				<div class="text"><?php echo $body_text; ?></div>
			</div>
		</section>
		<?php endif; ?>
		...
		<?php
		$news_query = new WP_Query( array( 'post_type' => 'post', 'posts_per_page' => 3, 'post_status' => 'publish' ) );
				...
				<div class="news_blocks">
					...
					<a href="<?php the_permalink(); ?>" class="news_block<?php echo has_post_thumbnail() ? '' : ' news_block--no-image'; ?>">
						<?php if ( has_post_thumbnail() ) : ?>
							<div class="news_block__image"><?php the_post_thumbnail( 'large' ); ?></div>
						<?php endif; ?>
